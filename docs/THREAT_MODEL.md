...
| Stripe webhook flood | Bridge rejects oversized bodies before parsing; per-source rate limit gates signature verification work (`WEBHOOK_RATE_LIMIT_PER_MIN`, returns HTTP 429 + Retry-After); standard L7 DoS still needs CDN/rate-limit upstream | `MAX_WEBHOOK_BYTES` / `MAX_PAYOUT_BYTES` + [`bridges/lib/limits.mjs`](../bridges/lib/limits.mjs) | Operational at the edge — production wants CDN + WAF upstream |
...
| Operator mints unlimited SSDC | `SSDC.mintShares` is `onlyTreasury`; treasury wallet is configured at init. Bridge layer additionally enforces a sliding 24h cap (`MAX_DAILY_MINT_USD`) so even a compromised treasury key is volume-bounded between alerts. | `SSDC.setTreasuryVault` + [`MultisigGuard.sol`](../contracts/commerce/MultisigGuard.sol) | Treasury can now be a `MultisigGuard` (demo: [`demos/multisig-operator.mjs`](../demos/multisig-operator.mjs)). Production should also pause-as-default + monitor `Transfer(from=0x0)` events. |
...
| Bridge operator key compromise → mass mint | Per-day USD-equivalent volume cap (`MAX_DAILY_MINT_USD` / `MAX_DAILY_PAYOUT_USD`) + per-source rate limit. Cap state is durable across restarts (loaded from the same idempotency directory as event nonces). Treasury wallet can be `MultisigGuard` (see [`demos/multisig-operator.mjs`](../demos/multisig-operator.mjs)) for the multi-sig escape path. | [`bridges/lib/limits.mjs`](../bridges/lib/limits.mjs) + [`MultisigGuard.sol`](../contracts/commerce/MultisigGuard.sol) | Production still wants `Transfer(to=…0)` reversal-pattern monitoring upstream of the bridge |
...
1. **On-chain STARK proof validation is wired but not bundled.** The chain commits to the proof's `proofHash`; cryptographic verification still happens off-chain with Winterfell. As of v0.8.0, [`demos/verify-onchain.mjs`](../demos/verify-onchain.mjs) shows the wiring to call an *upstream* Solidity verifier (`STARK_VERIFIER_ADDRESS`) — point it at a deployed verifier and you get end-to-end on-chain validation. The verifier itself still lives upstream in [`stateset-starks`](https://github.com/stateset/starks).
2. **Operator role is single-key in the *demo* deploy.** Closed in code as of v0.8.0 via [`MultisigGuard`](../contracts/commerce/MultisigGuard.sol) + [`demos/multisig-operator.mjs`](../demos/multisig-operator.mjs):
   - `SSDC.treasuryVault` → transferable via setter; demo proves a 2-of-3 mint path end-to-end.
   - `SSDC.owner`, `SetRegistry/PaymentBatch.owner`, `NAVOracle.owner` → all `transferOwnership(guard)` compatible.
   - `OrderEscrow.operator` and `FxOracle.operator` are `immutable` — pass the guard as constructor arg in `DeployLocal` / `DeploySepolia` to migrate.
   - `NAVOracle.authorizedAttestors` already has its own on-chain threshold via `setAttestationThreshold`; the multisig is for the admin surface.
3. **Bridge layer rate limits + per-day caps.** Closed in code as of v0.8.0 — both bridges enforce a sliding 24h USD-equivalent volume cap (`MAX_DAILY_MINT_USD` / `MAX_DAILY_PAYOUT_USD`, default 100k) and per-source request rate limits (`WEBHOOK_RATE_LIMIT_PER_MIN` / `PAYOUT_RATE_LIMIT_PER_MIN`). Both gates are fail-closed at startup and rebuild the 24h window from the durable idempotency directory so restarts don't reset state. Exhausted requests return HTTP 429 with `Retry-After`. Production operators still need monitoring on top — see [`bridges/lib/limits.mjs`](../bridges/lib/limits.mjs).
4. **No dispute escalation tier.** A dispute resolves in favor of one party once the operator picks. Production wants buyer-can-appeal logic, possibly to a second-tier arbiter. With `MultisigGuard` as the operator, the dispute decision is at least *jointly* signed, but the structure is still single-tier.
5. **Stripe chargeback / refund handling is operational.** The mint is irreversible from the bridge's perspective; if the buyer chargebacks, the operator is on the hook for the SSDC. The v0.8.0 daily cap bounds the worst-case loss to one cap-window's worth of mints, which is a partial mitigation but not a full one.
